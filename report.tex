...
\todo{fix} for something close. Luckily, we do have such an object for monoids:
the free monoid is more commonly known as the \emph{list}.
...
types the solver will be available for \todo{clarify}.
...
harder to find. In order to line things up correctly\todo{repetition}, we'll
need to compare the gap indices. This, however, presents our first problem:
...
\begin{figure*}
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      title={$(x_1 + x_2 + \ldots + x_n)^d$},
      xlabel={$d=$},
      x label style={at={(axis description cs:0.1,0.017)},anchor=south},
      ]
      \addplot coordinates{
        (1,5.042)
        (2,4.959)
  ...
  \begin{tikzpicture}
    \begin{axis}[
      width=0.35\textwidth,
      title={$(1 + x_1^d + x_2^d + \ldots + x_n^d)^d$},
      xlabel={$d=$},
      x label style={at={(axis description cs:0.1,0.017)},anchor=south},
      ]
      \addplot coordinates{
  ...
  \begin{tikzpicture}
    \begin{axis}[
      width=0.35\textwidth,
      title={$(x_1^d + \ldots + x_n^d) + (x_1^d + \ldots + x_n^d)$},
      xlabel={$d=$},
      x label style={at={(axis description cs:0.1,0.017)},anchor=south},
      ]
      \addplot coordinates{
  ...
  \end{tikzpicture}
  \ref{benchplots}
  \caption{Time (in seconds) to normalize and evaluate the given expressions.
    ($n = 5$ for each).}
  \label{benchmarks}
\end{figure*}
The benchmarks in figure~\ref{benchmarks} clearly show the efficiency gains of
the sparse representation over the dense. The sparse representation shines
especially when individual variables are raised to high powers: in these cases,
operations on the sparse representation are constant time, whereas on the dense
they scale with the particular power. Since we imagine this is a common case, we
believe it represents an important speed increase.

All of the benchmarks are performed on compiled Agda code: for interpreted code
(that's run at type-checking time), the sparse encoding is only faster for
pathological cases. Hopefully, sometime in the future Agda's reflection
mechanisms will be able to make use of compiled code: until then, the
performance benefits are only useful for application code, like the pedagogical
solutions mentioned below. Since the reflection component and the pedagogical
components were designed with modularity in mind, swapping in different
implementations for the solver backend is trivial.
...
\begin{principle}[Don't help the compiler]
  ...
  the compiler is.\todo{rephrase this sentence}
\end{principle}
...
to match expressions, how are we going to match the carrier type\todo{rephrase
  (parenthesize)}? Do we need the user to provide that function? Some kind of
matching logic (as in\cite{jedynak_simple_2018}), which would need to recognize
every constructor case, and build the corresponding \(\AgdaDatatype{Term}\)?

Maybe we go in another direction: we could search the subexpression to see if it
...
\subsection{Pedagogical Solutions} \label{pedagogical} \todo{Expand!}
\begin{figure*}[t]
  \centering
  \begin{tikzpicture}
