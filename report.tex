...
\ExecuteMetaData[PerformanceOfTypeChecking.tex]{dense-term-small}
...
\ExecuteMetaData[PerformanceOfTypeChecking.tex]{reduction}
...
\ExecuteMetaData[PerformanceOfTypeChecking.tex]{syntactic}
However the following expression, while \emph{definitionally} equal, fails the
syntactic check:
\ExecuteMetaData[PerformanceOfTypeChecking.tex]{definitional}
...
go ahead and reduce the terms on either side: this is disastrous! Not because we
have to pay for running the functions (which is bad in itself), but because it
will completely change the shape of the expression, destroying any opportunity
for syntactic equality on the \emph{subterms}! One bad reduction will cause a
cascade of them, meaning that we may have to reduce the entire expression to
normal form before it's definitionally equal on both sides.
...
\begin{principle}[To get syntactic equality, \emph{avoid} reduction for as long
  as possible]
  Since we know that the shape of the expressions on both sides of the equality
  are going to be similar, we want to \emph{maintain} that shape as much as
  possible. To that end, we go against the usual advice for proof code: we write
  our functions in such a way that they \emph{don't} reduce (as much as is
  possible). This is the key optimization which can lead us to massive speedups
  in code where evaluation is expensive.
\end{principle}
\subsubsection{Sparse Encoding}
Even with syntactic equality and everything else, the time it takes to check for
equality is still bounded by the size of the expression. The various sparse
encodings do actually help us out here, a little. The pyramid above, when
evaluated as a sparse polynomial, now becomes:
\begin{center}
\ExecuteMetaData[PerformanceOfTypeChecking.tex]{sparse-term}
\end{center}
Still worse than we'd like, for sure, but we don't now have to pay for the gaps!

This also ties in (in an important way) to maintaining syntactic equality:
remember, what we want to avoid is forcing a function on the carrier type. That
changes the shape of the expressions in an unpredictable way. We \emph{can}
force an expression on the power indices, though (\(x \AgdaSpace{}
\AgdaFunction{\textasciicircum{}} \AgdaSpace{} \AgdaNumber{2}\), for instance),
because both sides will have the same shape after forcing, preserving the
opportunity for syntactic equality. This is why avoiding gaps can lead to
serious speedups: it lets us postpone the call to operations on the carrier even
longer.
...
plus: 
\ExecuteMetaData[PerformanceOfTypeChecking.tex]{kleene}

And using this avoid the identity at the end of the list:
\ExecuteMetaData[PerformanceOfTypeChecking.tex]{no-ident}

Removing all of those identities finally gives us the following result:
\begin{center}
\ExecuteMetaData[PerformanceOfTypeChecking.tex]{small-eval}
\end{center}
...
\ExecuteMetaData[AbstractionAndFolds.tex]{para}
...
\ExecuteMetaData[AbstractionAndFolds.tex]{poly-foldR}
...
reflexive technique.
\footnotetext{These benchmarks were performed on Agda version 2.6-0fa9b13, with
  the Agda standard library at commit-3bd3334a9552490e396f73f96812105a27e5917b,
  on a 2016 MacBook Pro, with a 2.9 GHz Intel Core i7 and 16 GB of RAM.}
\begin{figure}[H]
  \newcounter{realxtickpos}
  \pgfplotsset{
      tick label style={font=\scriptsize},
      extra y tick style={grid=major},
      title style={at={(0.06,0.8)},anchor=west},
      xticklabel={%
...
\begin{figure}
  \ExecuteMetaData[ReflectionSection.tex]{nat-term}
  \caption{A Function To ``quote'' \(\AgdaDatatype{ℕ}\)}
  \label{nat-term}
\end{figure}

To cut down on it, we will use some pattern synonyms:
\ExecuteMetaData[ReflectionSection.tex]{synonyms}
These match visible and hidden arguments, respectively. Another common pattern
is applying several hidden arguments for Agda to infer: we can write a function
to do this for us, as well.
\ExecuteMetaData[ReflectionSection.tex]{infer-hidden}
...
The next problem with the reflected AST is that it's far less \emph{typed} than
it could be. To be clear, it doesn't break type safety: the generated program is
still type-checked, but you can generate code with a type error in it without
any difficulty. On the other end, the reflected AST doesn't contain as much type
information as it could, which is often an annoyance.
...
All of the next section is parameterized by the number of variables in the
expression, so we can use an anonymous module:
\ExecuteMetaData[ReflectionSection.tex]{numvars}
...
It is only by a very lucky coincidence that we can avoid all of this. The trick
is demonstrated in figure~\ref{to-expr}. Notice that all of the other cases are
spoken for: in a correctly constructed expression, if no other clause matches,
then what's left \emph{has} to be a constant. So we just wrap it up in the
constant constructor!

\begin{figure*}
  \centering
  \ExecuteMetaData[ReflectionSection.tex]{to-expr}
  \caption{Converting from the Reflection AST to the Expr AST}
  \label{to-expr}
\end{figure*}

You'll notice that when we fail to find a match we assume that what we must have
is a constant, so we just wrap it up as if it were one.
...
\subsection{Interface for Partial Automation}
We also provide an interface intended for use in combination with larger proofs.
The example in figure~\ref{partial-auto} is perhaps a little silly (why not just
use \(\AgdaMacro{solve}\)?), but it demonstrates the general technique.
\begin{figure*}
  \centering
  \ExecuteMetaData[ReflectionSection.tex]{partial-auto}
  \caption{A Partially Automated Proof}
  \label{partial-auto}
\end{figure*}
...
\begin{figure*}[t]
  \centering
  \begin{tikzpicture}
    \node (n1) at (0,5) {$(x + y \times 1) + 3$};
    \node (n2) at (2.5,5) {$(x + y) + 3$};
    \draw (n1) to[out=90, in=90] node[above, fill=none] {$\times-\text{identity} (y)$} (n2);
    \node (n3) at (5,5) {$(y + x) + 3$};
    \draw (n2) to[out=270, in=270] node[below, fill=none] {$+-\text{commutative} (x, y)$} (n3);
    \node (n4) at (7.5,5) {$3 + (y + x)$};
    \draw (n3) to[out=90, in=90] node[above, fill=none] {$+-\text{commutative} ((y + x), 3)$} (n4);
    \node (n5) at (10,5) {$(3 + y) + x$};
    \draw (n4) to[out=270, in=270] node[below, fill=none] {$+-\text{assoc} (3, y, x)$} (n5);
    \node (n6) at (12.5,5) {$((2 + 1) + y) + x$};
    \draw (n5) to[out=90, in=90] node[above, fill=none] {$+-\text{eval} (2, 1)$} (n6);
  \end{tikzpicture}
...
\begin{figure}
  \centering
  \begin{tikzpicture}
    \node (n1) at (-2,7.5) {$2 + 1 + y + x * 1$};
    \node (n2) at (-2,6) {$2 + 1 + y + x$};
    \node (n3) at (2,6) {$1 + 2 + y + x$};
    \node (n4) at (0,4.5) {$3 + y + x$};
    \node (n5) at (0,3) {$y + 3 + x$};
    \node (n6) at (0,1.5) {$y + x + 3$};
    \node (n7) at (0,0) {$x + y + 3$};
    \draw[-stealth] (n1) to (n2);
    \draw[-stealth] (n2) to (n4);
    \draw[-stealth] (n4) to (n3);
    \draw[-stealth] ([xshift=-0.2cm] n4.south) to ([xshift=-0.2cm] n5.north);
    \draw[-stealth] ([xshift=-0.2cm] n5.south) to ([xshift=-0.2cm] n6.north);
    \draw[-stealth] ([xshift=-0.2cm] n6.south) to ([xshift=-0.2cm] n7.north);
    \draw[-stealth] ([xshift=0.2cm] n5.north) to ([xshift=0.2cm] n4.south);
    \draw[-stealth] ([xshift=0.2cm] n6.north) to ([xshift=0.2cm] n5.south);
    \draw[-stealth] ([xshift=0.2cm] n7.north) to ([xshift=0.2cm] n6.south);
  \end{tikzpicture}
...
\ExecuteMetaData[CorrectByConstruction.tex]{list-def}
\ExecuteMetaData[CorrectByConstruction.tex]{vec-def}
...
concatenation:
\ExecuteMetaData[CorrectByConstruction.tex]{list-app}
\ExecuteMetaData[CorrectByConstruction.tex]{vec-app}
...
the list:
\ExecuteMetaData[CorrectByConstruction.tex]{list-proof}
