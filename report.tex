...
To improve on this bound, we can rely on Agda's support for a small number of
built-in functions on natural numbers. Depending on the compiler backend, these
functions switch to a faster representation at runtime, while still relying on
the unary implementation in Agda for proofs. For our case, then, we can juggle
both the unary implementation and built-in, allowing the faster representation
to kick in a runtime, and aggressively using \(\AgdaFunction{erase}\) in
combination with \(\AgdaFunction{inspect}\) to justify its use. The
justifications are as follows:
\ExecuteMetaData[EfficiencyInIndexedTypes.tex]{fast-cmp-hom}
And the implementation can be seen in figure~\ref{fast-cmp}.
