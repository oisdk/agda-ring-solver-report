...
\subsection{Free Objects and Canonical Forms}
In both the monoid and ring solver, we will make use of the \emph{canonical
  forms} of expressions in each algebra.
\begin{definition}[Canonical Forms]
  The canonical form of an expression is a representation of that expression
  such that any two \emph{equivalent} expressions have the same representation.
\end{definition}
The basic idea is to convert each side of the equation to their canonical forms,
and check if those forms are equal.

To convert to the canonical form, we'll use a related concept: the free object.
\begin{definition}[Free Objects]
  ...
  Crucially, it also must have the property that \emph{it is always in canonical
  form}.
\end{definition}
So now we can accomplish ``canonicalization'' by converting an expression to the
free object, and then the free object back to an expression.

For monoids, the free object is \emph{lists}.
...
solver are as follows: a canonical form, a concrete representation of
...
While this form solves our efficiency problem, does it prevent redundant
...
verified: here, we prove that the polynomials are always in normal form.

Disallowing zeroes is a strong enough condition to ensure normal form: without
them, there can be no trailing zeroes, and all gaps must be represented by power
indices. To check for zero, we require the user supply a decidable predicate on
the coefficients. This changes the module declaration like so:
...
terminate, it isn't structurally decreasing in its arguments. To make it
structurally decreasing, and therefore show the compiler it terminates, we'll
use a well-known optimization for functional languages called ``call-pattern
...
termination to the compiler. \todo{Maybe explain better?}
...
argument to solve is something of an implementation detail. \todo{rephrase}
...
the one for lists.\todo{Show that AST for ring expressions here!} To that end,
we'll need build the \(\AgdaDatatype{Term}\) which will construct it for us.
