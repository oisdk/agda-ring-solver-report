...
\subsection{The Algebra of Programming}
The structure of the homomorphism proofs can be seen in figure~\ref{hom-struct}.
For every function \(f\) that we want to use, we'll have to prove that
\(\AgdaFunction{⟦\_⟧}\) forms a homomorphism from \(\boxed{f}\) to \(f\).

\begin{figure}
  \centering
  \begin{tikzcd}[cramped]
    \mathsf{F} \AgdaDatatype{Poly} \; i \ar[r, "\boxed{f}"] \ar[d, "\AgdaFunction{⟦\_⟧}"'] &
    \AgdaDatatype{Poly} \; i \ar[d] \ar[d, "\AgdaFunction{⟦\_⟧}"] \\
    \mathsf{F} \AgdaDatatype{Vec} \; \AgdaDatatype{Carrier} \; i \rightarrow
    \AgdaDatatype{Carrier} \ar[r, "f"] & \AgdaDatatype{Vec} \;
    \AgdaDatatype{Carrier} \; i \rightarrow \AgdaDatatype{Carrier}
  \end{tikzcd}
  \caption{General Structure for Homomorphism Proofs}
  \label{hom-struct}
\end{figure}

We'll use techniques similar to those in the algebra of
programming\cite{bird_algebra_1997} (which has been adapted to the dependently
typed setting\cite{mu_algebra_2009}). Taking \(\AgdaFunction{⊟}\) as an example,
because we wrote it as a fold, we can use the following combinator to build up
its homomorphism proof:
\ExecuteMetaData[Poly.tex]{aopa}
Reminiscent of the relational fold in\cite{mu_algebra_2009}, this builds up a
relation between list and a fold over that list. In this way, it's similar to
\(\AgdaFunction{foldr}\) for proofs: it removes the explicit recursion, allowing
us to focus on the two cases we need to handle.

In our case, though, we can go further: negation (along with all of the other
functions defined as folds) isn't concerned with the details of normalization or
the power index. It only transforms expressions that look like this:

\[ x + xs\times\rho \]

While we're ignoring the gaps in the exponents, and the other details (like the
proofs of normal form), proving homomorphism on this expression \emph{also}
proves homomorphism on the eventual more complex expression. The type for the
folds in Agda is as follows:

\ExecuteMetaData[Poly.tex]{poly-fold}

The \(\AgdaFunction{PolyF}\) type is the \(x + xs\times\rho\) expression from
above. Actually running the fold is simply a matter of applying all of the
normalization operators:
\ExecuteMetaData[Poly.tex]{run-fold}
...
\todo{Expand on the traced version, maybe clean it up? Also provide some
  examples.}
How to improve the quality of the results is yet to be explored: for instance,
most associativity rewrites can be elided, as readers tend to do those in their
heads. Also, in rewriting both sides to a normal form, we may reach definitional
equality far \emph{before} the actual normal form, leaving a chain of rewrites
in the middle which uselessly transforms forwards and back an expression.
Removing these useless rewrites may well be worthwhile.
\subsection{Isomorphisms}
Types themselves, of course, are a kind of rings. Under isomorphism, they
exhibit all the requires associativity, commutativity, and so on. 

In this way, our solver can be used to construct translation functions between
equivalent types, automatically and efficiently. I don't know of any serious
uses for this, but it is an interesting avenue to explore.
