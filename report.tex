...
The next link is between the AST and the normalized expression. We can convert
from the AST to a normal form like so:
...
\begin{figure*}
  \centering
  \begin{tikzcd}[cramped, column sep=-1cm]
    &
    \includegraphics[draft=false,height=0.8cm]{graphics/rhs-norm}
    \ar[r, Leftrightarrow, bend left=50, "{\AgdaField{refl}}" description] &
    \includegraphics[draft=false,height=0.8cm]{graphics/rhs-norm} & \\
    \includegraphics[draft=false,height=0.8cm]{graphics/lhs-ast}
    \ar[ur, start anchor=north, end anchor=west, "{\AgdaFunction{⟦\_⇓⟧}}"]
    \ar[dr, start anchor=south, end anchor={[yshift=4ex]west}, "{\AgdaFunction{⟦\_⟧}}"'] &
    &
    &
    \includegraphics[draft=false,height=0.8cm]{graphics/rhs-ast}
    \ar[ul, start anchor=north, end anchor=east, "{\AgdaFunction{⟦\_⇓⟧}}"']
    \ar[dl, start anchor=south, end anchor={[yshift=4ex]east}, "{\AgdaFunction{⟦\_⟧}}"] \\
    &
    \includegraphics[draft=false,height=0.8cm]{graphics/lhs-expr}
    \ar[uu, squiggly, no head, "{\AgdaFunction{correct}}" description] &
    \includegraphics[draft=false,height=0.8cm]{graphics/rhs-expr}
    \ar[uu, squiggly, no head, "{\AgdaFunction{correct}}" description] &
  \end{tikzcd}
\end{figure*}
...
tackle next. \todo{
  It's unclear how this gets us to the desired proof in the end. Maybe add a
  diagram like the one in \cite{hutchison_proving_2005}? Possibly expand it out
  a little, to include where reflection would fit in, and how the equivalent
  diagram for correct by construction (section~\ref{correct-by-constr}) differs.
}
