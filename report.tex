...
that in understood, cast your mind back to the implementation of Horner's rule
...
And if you think that looks bad, check out the term constructed by:
\[3 + 2x^2 + 4x^5 + 2x^7\]
...
The first thing we might notice is that the order of arguments to
\(\AgdaFunction{+}\) and \(\AgdaFunction{*}\) are somewhat unnatural. Why are
they not flipped?
\ExecuteMetaData[PerformanceOfTypeChecking.tex]{backwards-eval} 
This puts any known arguments on the left-hand-side. Taking the \(x^2 + 2\)
example again, because \(x \; \AgdaFunction{*}\) can't be reduced any further
(without knowing something about \(x\)), the big expression in
\(\AgdaFunction{obligation}\) is \emph{already} in normal form. If we had simply
swapped the arguments, we would have gotten the following (much smaller) normal
form:
\ExecuteMetaData[PerformanceOfTypeChecking.tex]{reduction}

So why don't we? Because reduction is expensive, and there's a way to avoid it:
Before the ``full'' typechecking algorithm is run, Agda performs a quick check
for syntactic equality. This is much more efficient than the full type checking
algorithm, so we should strive to encourage it as much as possible.
...
even more apparent here, with an \(15\)-factor speedup at \(d = 8\).
...
\begin{figure}[H]
  ...
  \begin{subfigure}[h]{\linewidth}
    \begin{tikzpicture}
      \begin{axis}[
        extra y ticks={35},
        title={(\subref{bench3}) $(1 + x_1^1 + x_2^2 + \ldots + x_n^n)^d$},
        ]
        \addplot[color=blue, densely dashed] coordinates{
          (1,6.05816)
          (2,5.42016)
