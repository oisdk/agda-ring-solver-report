...
All of the benchmarks are performed on compiled Agda code: for interpreted 

\todo{Switch the order of the reveal!}
Unfortunately, the sparse representation does not exhibit faster performance at
type-checking time. None of the optimizations (such as the fast comparison on
natural numbers, or elision of \(\AgdaDatatype{Acc}\)) are applied, and so we
don't see a benefit anywhere except with truly pathological cases.
